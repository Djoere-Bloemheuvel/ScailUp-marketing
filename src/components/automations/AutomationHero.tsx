  ...
  return (
    <section className="relative py-20 lg:py-32 overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-4">
        ...
        {/* Core Benefits Section - Simplified styling to match services section */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <Card 
              key={index} 
              className="group relative overflow-hidden cursor-default animate-fade-in transform transition-all duration-500 ease-out hover:scale-105 hover:-translate-y-2"
              style={{
                animationDelay: `${index * 0.2}s`,
                background: `
                  linear-gradient(135deg, rgba(255, 255, 255, 0.08) 0%, rgba(255, 255, 255, 0.02) 100%),
                  radial-gradient(ellipse 300px 150px at 50% 0%, rgba(59, 130, 246, 0.04) 0%, transparent 70%)
                `,
            ...
            >
              {/* Hover glow effect */}
              <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-30 transition-all duration-500 ease-out bg-gradient-to-br from-white/5 to-transparent" />
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${
                        benefit.accentColor.includes('blue') 
                          ? 'rgba(59, 130, 246, 0.2), rgba(147, 51, 234, 0.1)' 
                          : benefit.accentColor.includes('purple') 
                          ? 'rgba(147, 51, 234, 0.2), rgba(236, 72, 153, 0.1)'
                          : 'rgba(34, 197, 94, 0.2), rgba(59, 130, 246, 0.1)'
                      })`,
                      border: '1px solid rgba(255, 255, 255, 0.15)'
                    }}
                  >
                    <div className="w-full h-full rounded-xl flex items-center justify-center bg-black/40 backdrop-blur-sm">
                      <benefit.icon className="w-8 h-8 text-white/90 transition-all duration-300 group-hover:text-white" />
                    </div>
                  </div>
                </div>
                
                {/* Typography - cleaner styling */}
                <h3 className="text-xl font-bold text-white mb-4 group-hover:text-white transition-all duration-300">
                  {benefit.title}
                </h3>
                
                <p className="text-premium-silver/70 leading-relaxed text-sm group-hover:text-premium-silver/85 transition-all duration-300">
                  {benefit.description}
                </p>
              </CardContent>

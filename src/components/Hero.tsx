...
import AIMeshLines from './AIMeshLines';
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsLoaded(true);
      console.log('Hero animations triggered, isLoaded:', true);
    }, 100);
  ...
  useEffect(() => {
    console.log('Animation state changed, isLoaded:', isLoaded);
  }, [isLoaded]);

  return (
    <section 
      className="relative min-h-screen flex items-center justify-center px-4 py-12 bg-black overflow-hidden"
      style={{
        isolation: 'isolate',
        contain: 'layout style paint'
      }}
    >
      {/* Background isolation container - completely isolated */}
      <div 
      ...
      >
        ...
        <div 
          className="absolute inset-0 bg-gradient-to-b from-black via-black to-gray-900/20"
          style={{ zIndex: -40 }}
        ></div>
        
        {/* AI Mesh Lines - deepest background */}
        <div style={{ zIndex: -35, isolation: 'isolate' }}>
          <AIMeshLines />
        </div>
        
        {/* Glitch Background - isolated background effects */}
        <div style={{ zIndex: -30, isolation: 'isolate' }}>
          <GlitchBackground />
        </div>
      </div>

      {/* Floating elements layer - isolated from background */}
      <div 
        className="absolute inset-0"
        style={{
          zIndex: 10,
      ...
      >
        <div className="absolute top-20 left-20">
          <div 
            className="w-24 h-24 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm flex items-center justify-center nothing-float"
        ...
        <div className="absolute top-40 right-32">
          <div 
            className="w-16 h-16 rounded-lg border border-white/10 bg-white/5 backdrop-blur-sm nothing-float-delayed"
            style={{ willChange: 'transform' }}
          >
        ...
        <div className="absolute bottom-32 left-32">
          <div 
            className="w-20 h-8 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm nothing-float-slow"
            style={{ willChange: 'transform' }}
          >
        ...
        <div className="absolute top-60 right-20">
          <div 
            className="w-6 h-6 rounded-full bg-white/20 nothing-pulse"
            style={{ willChange: 'transform' }}
          ></div>
        </div>
        ...
        <div className="absolute bottom-40 right-40">
          <div 
            className="w-4 h-4 rounded-sm bg-premium-silver/30 nothing-pulse-delayed"
            style={{ willChange: 'transform' }}
          ></div>
        </div>
      </div>

      {/* AI Devices layer - isolated mid layer */}
      <div 
        className="absolute inset-0"
        style={{
          zIndex: 20,
      ...
      >
        ...
        <div className="absolute left-20 top-1/3 transform -translate-y-1/2">
          <div className="relative">
            <div 
              className="relative w-72 h-96 ai-device-float overflow-visible"
              style={{ willChange: 'transform' }}
            >
              <div 
        ...
        <div className="absolute right-20 top-2/3 transform -translate-y-1/2">
          <div className="relative">
            <div 
              className="relative w-72 h-96 rounded-3xl bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-xl border border-white/20 ai-device-float overflow-hidden"
              style={{ willChange: 'transform' }}
            >
      ...
      {/* Content Shield - Protection layer for text */}
      <div 
        className="absolute inset-0"
        style={{
          zIndex: 98,
      ...
      {/* Main Content Layer - Highest priority with explicit stacking context */}
      <div 
        className="relative max-w-5xl mx-auto text-center hero-content-layer"
        style={{
          zIndex: 100,
          isolation: 'isolate',
          contain: 'layout',
      ...
      >
        <div 
          className="max-w-4xl mx-auto hero-text-container"
          style={{
            zIndex: 101,
            position: 'relative',
            isolation: 'isolate'
        ...
        >
          ...
          <div 
            className="hero-debug-indicator"
            style={{
              position: 'absolute',
              top: '-20px',
              left: '50%',
          ...
          {/* Main heading with forced white color and text shadow */}
          <h1 
            className={`font-bold tracking-tight leading-[0.9] mb-2 hero-main-title ${isLoaded ? 'hero-fade-in hero-stagger-2' : ''}`}
            style={{
          ...
          {/* Subtitle with forced color */}
          <p 
            className={`text-2xl md:text-3xl lg:text-4xl mb-16 leading-relaxed font-light hero-subtitle ${isLoaded ? 'hero-fade-in hero-stagger-3' : ''}`}
            style={{
          ...
          {/* Buttons with explicit z-index */}
          <div 
            className={`flex flex-col sm:flex-row gap-6 justify-center items-center mb-20 hero-buttons ${isLoaded ? 'hero-fade-in hero-stagger-4' : ''}`}
            style={{
              zIndex: 103,
              position: 'relative'
            }}
          >
            ...
            <Button 
              variant="outline" 
              className="relative hover:text-white px-10 py-6 text-xl font-semibold rounded-full border-premium-silver/30 hover:border-premium-silver/60 transition-all duration-700 apple-button-hover group overflow-hidden bg-white/[0.03] backdrop-blur-sm hover:bg-white/[0.08]"
              style={{

...
const WorkflowStep = ({ step, delay, isLast, verticalOffset = 0 }: WorkflowStepProps) => {
  ...
  return (
    ...
    >
      <div className="relative flex flex-col items-center space-y-7 p-7 lg:p-11 transition-all duration-500 ease-out group-hover:transform group-hover:[transform:perspective(1200px)_rotateX(3deg)_rotateY(3deg)_translateY(-12px)] cursor-pointer">
        ...
        <div className="relative">
          <div className="w-23 h-23 lg:w-27 lg:h-27 rounded-full bg-gradient-to-br from-white via-white to-gray-100 flex items-center justify-center relative z-10 transition-all duration-700 group-hover:shadow-3xl group-hover:shadow-blue-500/30 group-hover:scale-110">
            <span className="text-3xl lg:text-4xl font-bold text-black tracking-tight relative" style={{ fontSize: '0.95em' }}>
              {step.number}
          ...
          {!isLast && (
            <div className="hidden lg:block absolute top-1/2 -right-1/2 w-full h-px">
              ...
              {/* Flowing accent */}
              <div className="absolute inset-0 w-7 h-full bg-gradient-to-r from-transparent via-white/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-[slide-connection_2s_ease-in-out_infinite]" />
            </div>
          )}
        </div>
        ...
        <div className="space-y-6 max-w-sm transition-all duration-500 ease-out group-hover:-translate-y-2">
          <h3 className="text-2xl lg:text-3xl font-bold text-white leading-tight tracking-tight transition-all duration-500 relative group-hover:-translate-y-1 group-hover:text-white" style={{ fontSize: '0.95em' }}>
            {step.title}
          ...
          <p className="text-white/60 leading-relaxed text-lg lg:text-xl font-light tracking-wide transition-all duration-500 group-hover:text-white/85 group-hover:scale-105" style={{ fontSize: '0.95em' }}>
            {step.subtitle}
          </p>
          ...
          <div className="flex items-center justify-center space-x-2 opacity-0 group-hover:opacity-100 transition-all duration-500">
            ...
            <span className="text-white/40 text-sm font-medium" style={{ fontSize: '0.95em' }}>Stap {step.number}</span>
            <div className="w-1.5 h-1.5 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full animate-pulse" style={{ animationDelay: '0.5s' }} />
          </div>
        </div>

        {/* Premium microinteraction elements */}
        <div className="absolute top-4 right-4 w-2.5 h-2.5 rounded-full bg-white/20 group-hover:bg-white/50 transition-all duration-500 group-hover:scale-125 opacity-0 group-hover:opacity-100" />
        <div className="absolute bottom-4 left-4 w-1.5 h-1.5 rounded-full bg-cyan-400/30 group-hover:bg-cyan-400/60 transition-all duration-500 group-hover:scale-150 opacity-0 group-hover:opacity-100" />
        ...
        {!isLast && (
          <div className="lg:hidden absolute -bottom-10 left-1/2 transform -translate-x-1/2 w-px h-19">
      ...
      <div className="absolute inset-4 rounded-3xl bg-gradient-to-br from-blue-500/20 via-purple-500/15 to-cyan-500/10 opacity-0 group-hover:opacity-100 transition-all duration-700 -z-10 blur-3xl transform translate-y-7 group-hover:translate-y-11 group-hover:scale-110" />
